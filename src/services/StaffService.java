...
public class StaffService {
	...
	private void initialiseData() {
		staffList.add(new Staff("S001", "Teoh Yik Tze", "CEO", "Male", 696969));
		staffList.add(new Staff("S002", "Ooi Zhen Yi", "Manager", "Male", 8000000));
		staffList.add(new Staff("S003", "Beh Jie Shen", "Intern", "Male", 2));
		staffList.add(new Staff("S004", "Teoh Zhao Bin", "Medic", "Male", 2500));
		staffList.add(new Staff("S005", "Sum Ting Wong", "Impatient", "Female", 0));
	}
	...
	public OperationResult<Void> addStaff(String newId, String newName, String newDesignation, String newSex, String rawSalary) {
		if (staffList.size() >= MAX_STAFF) {
	        return new OperationResult<>(false, "Cannot add staff. Staff capacity reached (Max 100).", null);
	    }
		for (Staff staff : staffList) {
			if (newId.equalsIgnoreCase(staff.getId()))
				return new OperationResult<>(false, "Staff ID " + newId + " already exists.", null);
		}
		if (newName == null || newName.isBlank())
			return new OperationResult<>(false, "Please enter a name", null);
		if (newDesignation == null || newDesignation.isBlank())
			return new OperationResult<>(false, "Please enter a designation", null);
		if (newSex == null || newSex.isBlank() || newSex.equalsIgnoreCase("Select Gender"))
			return new OperationResult<>(false, "Please enter a sex", null);
		...
		if (!InputValidator.isPositive(salary))
			return new OperationResult<>(false, "Salary must be a positive number.", null);
	...
	public OperationResult<Void> removeStaff(Staff staff) {
		if (staff == null)
			return new OperationResult<>(false, "No Staff selected", null);

		if (staffList.remove(staff)) {
			return new OperationResult<>(true, "Staff " + staff.getId() + " deleted. This action cannot be undone.",
					null);
		}
	...
	public OperationResult<Void> updateStaff(Staff staff, String newName, String newDesignation, String newSex,
			String rawSalary) {
		if (staff == null)
	...
	public OperationResult<ArrayList<Staff>> searchStaff(String searchField) {
		...
		for (Staff staff : staffList) {
			...
			if (designation != null && !designation.isBlank()
					&& !staff.getDesignation().toLowerCase().contains(designation.toLowerCase())) {
				match = false;
			}
